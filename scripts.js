...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function closeLoginModal() {
    ...
    if (!document.querySelector('.modal.active') && !sideNav.classList.contains('open')) {
  ...
  async function renderAuthUI() {
    ...
    if (s.authenticated) {
      ...
      badge.textContent = s.username || "کاربر";
  ...
  loginBtn?.addEventListener("click", async () => {
    ...
    if (!username || !password) {
      loginError.textContent = "نام کاربری و گذرواژه الزامی است.";
    ...
    try {
      ...
      if (r.ok && j.success) {
        ...
        loginError.textContent = j.message || "ورود ناموفق بود.";
        loginError.style.display = "block";
      }
    } catch (e) {
